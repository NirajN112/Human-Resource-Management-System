...
const Signin = ({ close }) => {
  ...
  const [adminType, setAdminType] = useState('employee'); // Defaulting to 'employee'
  ...
  const onSubmit = async (e) => {
    ...
    const res = await signIn("credentials", {
      ...
      adminType: adminType, // Pass selected role to NextAuth backend config
      redirect: false,
    });
    
    if (res?.ok) {
      // Redirects to either /admin/dashboard or /employee/dashboard
      window.location.href = `/${adminType}/dashboard`;
    } else {
      setisLoadingSignText(false);
      setMessage("Invalid credentials or role mismatch. Please check your inputs.");
    }
  };
      ...
      <div className="w-full max-w-md rounded-xl bg-gray-800 p-8 shadow-2xl border border-gray-700">
        ...
        <div className="text-center mb-8">
          {close && (
            <div className="flex flex-row-reverse cursor-pointer">
              <X className="w-6 h-6 text-gray-400 hover:text-white" onClick={() => { close() }} />
            </div>
          )}
          <h1 className="text-3xl font-bold text-green-400">HRMS Portal</h1>
          <p className="text-gray-400 mt-2">Sign in to manage your professional profile & payroll.</p>
        </div>

        <form onSubmit={onSubmit} className="space-y-6">

          {/* Admin Type / Role Dropdown */}
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="employee" className="bg-gray-700 text-white">Regular Employee</option>
                <option value="admin" className="bg-gray-700 text-white">HR Admin</option>
              </select>
        ...
        <div className="mt-6 text-center space-y-2">
          ...
          <p className="text-gray-400 text-sm">
            New employee?{" "}
            <a href="/signup" className="text-green-400 hover:underline font-medium transition duration-150">
              Register Account
            </a>
          </p>
        </div>
